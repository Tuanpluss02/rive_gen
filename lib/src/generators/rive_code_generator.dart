...
class _AssetClassGenerator {
  ...
  String generate() {
    ...
    return '''
/// Generated asset class for ${asset.name}.riv
class $className {
  /// The path to the Rive asset
  static const String path = '${asset.path}';
  
  /// The asset name
  static const String name = '${asset.name}';
  
  // TODO: Add artboards, animations, and state machines
}
''';
  }
}
...
class _ControllerGenerator {
  ...
  String generate() {
    final className =
        '${config.classPrefix}${asset.className}Controller${config.classSuffix}';

    return '''
/// Generated controller class for ${asset.name}.riv
class $className {
...
${assets.map((asset) {
      ...
      return "  /// ${asset.name}.riv asset\\n  static const $className $propertyName = $className();";
    }).join('\n\n')}
}
''';
  }
